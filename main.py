...
def main(pdb, query_chain, partner_chain, sr, result_file, mi, scrwl, mutants):
    ...
    intercaat_result, intercaat_result_changed,positions  = intercaatRun(pdb, query_chain, partner_chain, sr, mi)
    ...
    for mutAA in mutants:
        for key in positions:
            if key[:3] != mutAA:
                ...
                mutant_interactions = mutantIntercaatRun(mutant, query_chain, partner_chain, mutposition, sr)
                print("\n\n", key, wt_interactions,mutant_interactions, "\n\n")
                results[key] = results[key] + [f"{mutAA} {wt_interactions} {mutant_interactions}"]
                if mutant_interactions and key not in extened_interface:
                    extened_interface.append(key)
    outputWriter(result_file, pdb, query_chain, partner_chain, intercaat_result, intercaat_result_changed, extened_interface, results, positions)
    return

def outputWriter(result_file, pdb, query_chain, partner_chain, intercaat_result, intercaat_result_changed, extened_interface, resultsi, positions):
